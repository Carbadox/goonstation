...
/area
			...
			if (enteringMobs.len > 0)
				for (var/mob/enteringM in enteringMobs) //each dumb mob
					...
					if (enteringM.ckey && enteringM.client)
						...
						if( sanctuary && !blocked && !(get_area(oldloc).sanctuary))
			...
			if (exitingMobs.len > 0)
				for (var/mob/exitingM in exitingMobs)
					if (exitingM.ckey && exitingM.client && exitingM.mind)
						...
						if( sanctuary && !blocked && !(get_area(exitingM).sanctuary) )
	...
	proc/calculate_area_cleanliness()
		...
		for (var/turf/simulated/T in src.contents)
			dirty = 0
			total_count++
			for (dirtyStuff in T.contents)
				dirty = 1
				break
			if (!dirty)
				clean_count++
		if (total_count == 0 || clean_count == 0) return 0
		else return get_percentage_of_fraction_and_whole(clean_count,total_count)

	proc/pickAmbience()
		switch(src.name)
			if ("Chapel") sound_fx_1 = pick('sound/ambience/station/Chapel_FemaleChoir.ogg','sound/ambience/station/Chapel_ChoirTwoNote1.ogg','sound/ambience/station/Chapel_ChoirTwoNote2.ogg','sound/ambience/station/Chapel_HighFemaleSolo.ogg','sound/ambience/station/Chapel_DeathsCall.ogg')
